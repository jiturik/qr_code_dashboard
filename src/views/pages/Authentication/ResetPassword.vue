...
export default {
          ...
          try {
            ...
            if (response.data.status) {
              ...
              this.$toast({
                ...
                props: {
                  title: response.data.message || "Passoword reset failed",
                  icon: "XCircleIcon",
                  variant: "failure",
                },
              });
  ...
  beforeMount() {
    ...
    const token = url_search.get("token");
    const expiry_timestamp = url_search.get("expiry_timestamp");
    if (!token || !expiry_timestamp) {
      alert("Invalid Link");
      this.$router.replace("/");
      return;
    } else if (parseInt(expiry_timestamp) < moment().unix()) {
      alert("Link Expired");
      this.$router.replace("/");
      return;
    }
